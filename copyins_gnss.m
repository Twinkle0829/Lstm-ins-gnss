function [nav_e] = copyins_gnss(imu,imudata, gnss,gnssdata, att_mode)
if nargin < 3, att_mode  = 'quaternion'; end
%
%lstm_data.wb=imudata.wb(:,:);
fprintf('wb integration... \n');
%lstm_data.fb=imudata.fb(:,:);
fprintf('fb integration... \n');

%% ZUPT detection algorithm ZUPT 检测
...
%lstm_data.t=imudata.t;
% Length of GNSS time vector GNSS时间矢量的长度
LG = length(gnssdata.h);
...
roll_e(1)  = imudata.ini_align(1);
pitch_e(1) = imudata.ini_align(2);
yaw_e(1)   = imudata.ini_align(3);
...
% Initialize estimates at INS time = 1
h_e(1)   = gnssdata.h(1);
lat_e(1) = gnssdata.lat(1);
lon_e(1) = gnssdata.lon(1);
...
fb_corrected = (imudata.fb(1,:)' + ab_dyn );
fn = (DCMbn * fb_corrected);

% Vector to update matrix F
upd = [gnssdata.vel(1,:) gnssdata.lat(1) gnssdata.h(1) fn'];

% Update matrices F and G
[kf.F, kf.G] = F_update(upd, DCMbn, imu);

[RM,RN] = radius(gnssdata.lat(1));
Tpr = diag([(RM + gnssdata.h(1)), (RN + gnssdata.h(1)) * cos(gnssdata.lat(1)), -1]);  % radians-to-meters
...
for i = 2:LI
    ...
    dti = imudata.t(i) - imudata.t(i-1);
    
    % Inertial sensors corrected with KF biases estimation
    wb_corrected = (imudata.wb(i,:)' + gb_dyn );
    fb_corrected = (imudata.fb(i,:)' + ab_dyn );
...
%    printf('vel integration... \n');
    ...
    gdx =  find (gnss.t >= (imudata.t(i) - gnss.eps) & gnss.t < (imudata.t(i) + gnss.eps));
...
%         gdx 
        ...
        zp = Tpr * ([lat_e(i); lon_e(i); h_e(i);] - [gnssdata.lat(gdx); gnssdata.lon(gdx); gnssdata.h(gdx);]) ...
            + (DCMbn * gnss.larm);
...
nav_e.t     = imudata.t(1:i, :);    % INS time vector
